...
verbose = False  # whether to print updates of the partitioning
...
delta_entropy_moving_avg_window = 2#3  # width of the moving average window for the delta entropy convergence criterion
...
while not optimal_num_blocks_found:
    ...
    for itr in range(max_num_nodal_itr):
        ...
        oe = overall_entropy = compute_overall_entropy(interblock_edge_count, block_degrees_out, block_degrees_in, num_blocks, N, E, use_sparse_matrix)
        if verbose:
            print("Itr: {}, number of nodal moves: {}, delta S: {:0.5f}, oe: {}".format(itr, num_nodal_moves,
        ...
        if itr >= (
            ...
            if not (np.all(np.isfinite(old_overall_entropy))):  # golden ratio bracket not yet established
                if (-np.mean(itr_delta_entropy[(itr - delta_entropy_moving_avg_window + 1):itr]) < (
                    ...
                    if verbose:
                        print("golden ratio bracket not yet established: ",
                            -np.mean(itr_delta_entropy[(itr
                                - delta_entropy_moving_avg_window + 1):itr]), delta_entropy_threshold1 * overall_entropy)
                    break
            else:  # golden ratio bracket is established. Fine-tuning partition.
                if (-np.mean(itr_delta_entropy[(itr - delta_entropy_moving_avg_window + 1):itr]) < (
                    delta_entropy_threshold2 * overall_entropy)):
                    if verbose:
                        print("golden ratio bracket is established: ", -np.mean(itr_delta_entropy[(itr - delta_entropy_moving_avg_window + 1):itr]))
                    break
