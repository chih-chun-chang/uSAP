...
template <typename W>
void Graph<W>::_propose_new_partition_block(
  size_t r,
  const std::vector<size_t>& partitions,
  size_t& s,
  W& k_out,
...
template <typename W>
void Graph<W>::_propose_new_partition_nodal(
  size_t r,
  size_t ni,
  size_t& s,
  W& k_out,
...
template <typename W>
void Graph<W>::_compute_new_rows_cols_interblock_edge_count_block(
  size_t r,
  size_t s,
  std::vector<W>& M_r_row,
  std::vector<W>& M_s_row,
...
template <typename W>
void Graph<W>::_compute_new_rows_cols_interblock_edge_count_nodal(
  size_t r,
  size_t s,
  size_t ni,
  std::vector<W>& M_r_row,
  ...
  std::vector<W>& s_col)
{
  ...
  for (size_t v = 0; v < _num_blocks; v++) {
    if (r_row[v] != 0) {
      delta_entropy += r_row[v] * std::log(static_cast<float> (r_row[v]) / 
        (_block_degrees_in[v] * _block_degrees_out[r]));
    }
    if (s_row[v] != 0) {
      delta_entropy += s_row[v] * std::log(static_cast<float> (s_row[v]) / 
        (_block_degrees_in[v] * _block_degrees_out[s]));
    }
    if (v != r && v != s) {
      if (r_col[v] != 0) {
        delta_entropy += r_col[v] * std::log(static_cast<float> (r_col[v]) / 
          (_block_degrees_out[v] * _block_degrees_in[r]));
      }
      if (s_col[v] != 0) {
        delta_entropy +=  s_col[v] * std::log(static_cast<float> (s_col[v]) / 
          (_block_degrees_out[v] * _block_degrees_in[s]));
      }
    }
  }
...
template <typename W>
void Graph<W>::_carry_out_best_merges(
  const std::vector<size_t>& bestMerges,
  const std::vector<int>& best_merge_for_each_block,
  size_t B_to_merge,
  std::vector<size_t>& block_map,
  std::vector<size_t>& remaining_blocks) 
{
  ...
  _num_blocks = _num_blocks - B_to_merge;
  ...
  std::vector<W>& M_r_row)
{
  ...
  for (size_t i = 0; i < _num_blocks; i++) {
    ...
    for (size_t v = 0; v < _num_blocks; v++) {
      if (M_r_row[v] != 0) {
        data_S -= M_r_row[v] * std::log(M_r_row[v] / 
          (float)(_block_degrees_out[i] * _block_degrees_in[v]));
      }
    }
  } 
  ...
  float model_S = (float)(_E * (1 + model_S_term) * std::log(1 + model_S_term)) - 
                    (model_S_term * std::log(model_S_term)) + (_N * std::log(_num_blocks));
